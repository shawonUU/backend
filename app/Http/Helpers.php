    ...
    function get_cached_products($category_id = null)
    {
        ...
        if ($category_id != null) {
            return Cache::remember('products-category-' . $category_id, 86400, function () use ($category_id, $products) {
                ...
                return $products->whereIn('category_id', $category_ids)->latest()->take(8)->get();
            });
        } else {
            return Cache::remember('products', 86400, function () use ($products) {
                return $products->latest()->take(8)->get();
            });
        }
    }
}
...
if (!function_exists('seller_base_carrier_list')) {
    function seller_base_carrier_list($owner_id)
    {
        ...
        //  dd($carts);
        if (count($carts) > 0) {
            $zone = $carts[0]['address'] ? \App\Models\Country::where('id',$carts[0]['address']['country_id'])->first()->zone_id : null;
            //  dd($zone);
